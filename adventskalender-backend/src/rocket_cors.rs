...
/// ```rust
/// use std::str::FromStr;
/// use adventskalender_backend::rocket_cors::AllowedMethods;
///
...
/// # Examples
/// ```rust
/// use adventskalender_backend::rocket_cors::AllowedHeaders;
///
...
/// ## Pure default
/// ```rust
/// let default = adventskalender_backend::rocket_cors::CorsOptions::default();
/// ```
///
/// ## JSON Examples
/// ### Default
...
fn origin(request: &Request<'_>) -> Result<Option<Origin>, Error> {
    match Origin::from_request_sync(request) {
        Outcome::Forward(_status) => Ok(None),
...
impl rocket::route::Handler for CatchAllOptionsRouteHandler {
    async fn handle<'r>(
        ...
        let guard: Guard<'_> = match request.guard().await {
            ...
            Outcome::Forward(_status) => unreachable!("Should not be reachable"),
        };
