...
impl Origin {
    ...
    ) -> request::Outcome<Self, crate::rocket_cors::Error> {
...
impl FromStr for Origin {
    type Err = crate::rocket_cors::Error;

    fn from_str(input: &str) -> Result<Self, Self::Err> {
        if input.to_lowercase() == "null" {
            Ok(Origin::Null)
        } else {
            match crate::rocket_cors::to_origin(input)? {
...
impl<'r> FromRequest<'r> for Origin {
    type Error = crate::rocket_cors::Error;
...
#[derive(Debug)]
pub struct AccessControlRequestMethod(pub crate::rocket_cors::Method);
    ...
    pub fn from_request_sync(
        ...
        match request.headers().get_one("Access-Control-Request-Method") {
            Some(request_method) => match Self::from_str(request_method) {
                ...
                Err(_) => Outcome::Error((
                    Status::BadRequest,
                    crate::rocket_cors::Error::BadRequestMethod,
                )),
            },
            None => Outcome::Forward(Status::Ok),
        }
...
impl FromStr for AccessControlRequestMethod {
    ...
    fn from_str(method: &str) -> Result<Self, Self::Err> {
        Ok(AccessControlRequestMethod(
            crate::rocket_cors::Method::from_str(method)?,
        ))
    }
}
...
mod tests {
    ...
    static ACCESS_CONTROL_REQUEST_METHOD: HeaderName = http::header::ACCESS_CONTROL_REQUEST_METHOD;
