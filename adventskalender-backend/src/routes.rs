...
#[derive(Serialize)]
pub struct PickableParticipant {
    /// TODO
...
#[get("/participants/pickable")]
pub async fn get_list_of_pickable_participants(
    db_connection_pool: &State<AdventskalenderDatabaseConnection>,
    _authenticated_user: AuthenticatedUser,
) -> Result<Json<HashMap<i32, PickableParticipant>>, Status> {
    use crate::models::Participant as DatabaseParticipant;
    use crate::schema::participants::dsl::{participants, id};
        ...
        .run::<_, diesel::result::Error, _>(|connection| {
                ...
                .load::<DatabaseParticipant>(connection)
            {
                Ok(all_participants) => {
                    ...
                    for current_participant in all_participants.iter() {
                        result_map.insert(current_participant.id, PickableParticipant {
                            first_name: current_participant.first_name.clone(),
                            last_name: current_participant.last_name.clone(),
                            can_be_changed: current_participant.won_on.is_some(),
                            is_pickable: current_participant.can_be_picked
                        });
                    }
...
#[put("/participants/pickable/<id>")]
pub async fn toggle_if_participant_is_pickable(
    _db_connection_pool: &State<AdventskalenderDatabaseConnection>,
    _authenticated_user: AuthenticatedUser,
    id: i32
) -> Status {
